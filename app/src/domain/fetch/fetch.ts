...
import { ILogger, LoggerFactory } from "./core/logger/Logger";
...
export async function fetchHabits(): Promise<FetchHabitsResult> {
  ...
  try {
    ...
    // リポジトリを取得
    const habitRepository = ServiceFactory.getService<HabitRepository>('habitRepository');
 ...
 * 特定のHabitデータを取得
 */
export async function fetchHabitById(pageId: string): Promise<FetchHabitResult> {
  const logger = LoggerFactory.getLogger();
  const startTime = Date.now();
  ...
  try {
    ...
    // データを取得
    const habit = await habitRepository.fetchHabitById(habitsDatabaseId, pageId);
